...
function Bigquery_cost_daily({ size }) {
  const [graphType, setGraphType] = useState("line"); // Default graph type is line

  // Static data representing day-over-day cost for different BigQuery services for the month of June
  const dummyData = {
    days: Array.from({ length: 30 }, (_, i) => `June ${i + 1}`),
    services: [
        { name: "BigQuery", data: [12, 34, 28, 15, 42, 21, 90, 20, 85, 22, 21, 30, 22, 21, 30, 24, 50, 60, 27, 80, 90, 30, 31, 30, 29, 31, 32, 33, 34, 35] },
        { name: "Cloud Storage", data: [32, 30, 33, 31, 35, 33, 37, 38, 36, 40, 39, 40, 41, 43, 45, 44, 47, 48, 46, 50, 51, 52, 50, 54, 53, 55, 57, 56, 59, 60] },
  ...
  const insight = "Cloud Storage shows the highest variability in daily costs.";
...
export default Bigquery_cost_daily;
