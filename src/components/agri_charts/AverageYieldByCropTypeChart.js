import React, { useEffect, useState } from 'react';
import axios from 'axios';
import ReactECharts from 'echarts-for-react';
import Dropdown from 'react-bootstrap/Dropdown';
import DropdownButton from 'react-bootstrap/DropdownButton';

const AverageYieldByCropTypeChart = () => {
  const [chartData, setChartData] = useState({ cropTypes: [], averageYields: [] });
  const [graphType, setGraphType] = useState("line");
  const [insight, setInsight] = useState("");
  const [cta, setCta] = useState("");

  useEffect(() => {
    const fetchData = async () => {
      const config = {
        method: 'get',
        url: 'https://ig.gov-cloud.ai/pi-get-data-quarkus/v1.0/bigQuery/66b20fbfb7ea59463e79664b/data?size=10000',
        headers: {
          'Authorization': 'Bearer eyJhbGciOiJSUzI1NiIsInR5cCIgOiAiSldUIiwia2lkIiA6ICI3Ny1NUVdFRTNHZE5adGlsWU5IYmpsa2dVSkpaWUJWVmN1UmFZdHl5ejFjIn0..VYZcPeujLulFYl1ZOpdSXhafpRAM6qmWK48le-g30m9DGyfxsHqBSVERxBa1IqK5jTareyXvXWo2nVBCC67x_P6OfqtZRcMTUCaWQ3x_m6soScI6oCZBdEXjLPR8z_40ihhckO1aIdcqmI1ULlhrp_obPs00VdvKnXM8u9qZIhaOvb79-xYGx8CWd6LY2Foe_J_sW8YjRwEMeeVucqWBVdfJVpJ_gyuQgfigfV78ufjaKyIjy-FZ2t-ZGx5iT-ZsIaxK8eUqStljyQzDF5vZl3ZtJrdDgPZPDgU0ntbllSFSDMPt6i2BgvqQWh_ry4IV8mE2pPFYDicF3Rabzz982w'
        }
      };

      try {
        const response = await axios(config);
        const entities = response.data.model.entities;

        const cropTypes = [];
        const averageYields = [];

        entities.forEach(item => {
          const { average_yield, crop_types } = item.data;
          cropTypes.push(crop_types);
          averageYields.push(average_yield);
        });

        setChartData({ cropTypes, averageYields });

        // Generate insights and CTA
        generateInsights(cropTypes, averageYields);

      } catch (error) {
        console.error('Error fetching data:', error);
      }
    };

    fetchData();
  }, []);

  const generateInsights = (cropTypes, averageYields) => {
    let maxAverageYield = 0;
    let cropWithMaxYield = "";

    cropTypes.forEach((cropType, index) => {
      const averageYield = averageYields[index];
      if (averageYield > maxAverageYield) {
        maxAverageYield = averageYield;
        cropWithMaxYield = cropType;
      }
    });

    const insight = `${cropWithMaxYield} has the highest average yield.`;
    const cta = `Consider adopting best practices from ${cropWithMaxYield} to improve yield of other crops.`;

    setInsight(insight);
    setCta(cta);
  };

  const getOption = () => {
    return {
      title: {
        text: '',
      },
      tooltip: {
        trigger: 'axis',
        axisPointer: {
          type: 'shadow'
        },
        formatter: (params) => {
          const item = params[0];
          return `${item.name}<br/>${item.marker} ${item.seriesName}: ${item.value}`;
        }
      },
      legend: {
        data: ['Average Yield'],
      },
      xAxis: {
        type: 'category',
        data: chartData.cropTypes,
      },
      yAxis: {
        type: 'value',
      },
      series: [
        {
          name: 'Average Yield',
          type: 'line',
          data: chartData.averageYields,
          itemStyle: {
            color: (params) => {
              // Use HSL to generate different colors for each bar
              const hue = (params.dataIndex / chartData.cropTypes.length) * 360;
              return `hsl(${hue}, 70%, 50%)`;
            }
          }
        }
      ],
    };
  };

  return (
    <div className="average-yield-by-crop-type-chart-container" style={{ width: "100%", height: "100%" }}>
      <div style={{ textAlign: "end", marginRight: "25px" }}>
       
      </div>
      <div className="chart-container" style={{ height: "70%", width: "100%" }}>
        {graphType !== "table" ? (
          <ReactECharts option={getOption()} style={{ height: "100%", width: "100%" }} />
        ) : (
          <div className="table-container">
            <table>
              <thead>
                <tr>
                  <th>Crop Type</th>
                  <th>Average Yield</th>
                </tr>
              </thead>
              <tbody>
                {chartData.cropTypes.map((cropType, index) => (
                  <tr key={index}>
                    <td>{cropType}</td>
                    <td>{chartData.averageYields[index]}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
        )}
      </div>
      <div className="insight-cta-container" style={{ marginTop: "20px", textAlign: "center" }}>
        <p><strong>Insight:</strong> {insight}</p>
        <p><strong>Call to Action:</strong> {cta} <a href="#action-link">click here</a></p>
      </div>
    </div>
  );
};

export default AverageYieldByCropTypeChart;
