import React, { useEffect, useState } from 'react';
import axios from 'axios';
import ReactECharts from 'echarts-for-react';

const TotalYieldByCropSeasonChart = () => {
  const [data, setData] = useState([]);

  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(
          'https://ig.gov-cloud.ai/pi-get-data-quarkus/v1.0/bigQuery/66b2095eb7ea59463e796647/data?size=10000',
          {
            headers: {
              Authorization: 'Bearer eyJhbGciOiJSUzI1NiIsInR5cCIgOiAiSldUIiwia2lkIiA6ICI3Ny1NUVdFRTNHZE5adGlsWU5IYmpsa2dVSkpaWUJWVmN1UmFZdHl5ejFjIn0..VYZcPeujLulFYl1ZOpdSXhafpRAM6qmWK48le-g30m9DGyfxsHqBSVERxBa1IqK5jTareyXvXWo2nVBCC67x_P6OfqtZRcMTUCaWQ3x_m6soScI6oCZBdEXjLPR8z_40ihhckO1aIdcqmI1ULlhrp_obPs00VdvKnXM8u9qZIhaOvb79-xYGx8CWd6LY2Foe_J_sW8YjRwEMeeVucqWBVdfJVpJ_gyuQgfigfV78ufjaKyIjy-FZ2t-ZGx5iT-ZsIaxK8eUqStljyQzDF5vZl3ZtJrdDgPZPDgU0ntbllSFSDMPt6i2BgvqQWh_ry4IV8mE2pPFYDicF3Rabzz982w',
            },
          }
        );
        setData(response.data.model.entities);
      } catch (error) {
        console.error('Error fetching data:', error);
      }
    };

    fetchData();
  }, []);

  const getOption = () => {
    const districts = Array.from(new Set(data.map((item) => item.data.district)));
    const cropSeasons = Array.from(new Set(data.map((item) => item.data.crop_season)));

    const series = cropSeasons.map((season) => {
      return {
        name: season,
        type: 'bar',
        stack: 'total',
        data: districts.map((district) => {
          const entry = data.find((item) => item.data.district === district && item.data.crop_season === season);
          return entry ? entry.data.total_yield : 0;
        }),
      };
    });

    return {
      title: {
        text: '',
      },
      tooltip: {
        trigger: 'axis',
      },
      legend: {
        data: cropSeasons,
      },
      xAxis: {
        type: 'category',
        data: districts,
      },
      yAxis: {
        type: 'value',
        name: 'Total Yield (tons)',
      },
      series: series,
    };
  };

  return <ReactECharts option={getOption()} style={{ height: '400px', width: '100%' }} />;
};

export default TotalYieldByCropSeasonChart;
