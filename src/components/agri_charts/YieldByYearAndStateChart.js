import React, { useEffect, useState } from 'react';
import axios from 'axios';
import ReactECharts from 'echarts-for-react';

const YieldByYearAndStateChart = () => {
  const [chartData, setChartData] = useState({ years: [], states: {}, stateNames: [] });

  useEffect(() => {
    const fetchData = async () => {
      const config = {
        method: 'get',
        url: 'https://ig.gov-cloud.ai/pi-get-data-quarkus/v1.0/bigQuery/66b21045311fa40776ef57ee/data?size=10000',
        headers: {
          'Authorization': 'Bearer eyJhbGciOiJSUzI1NiIsInR5cCIgOiAiSldUIiwia2lkIiA6ICI3Ny1NUVdFRTNHZE5adGlsWU5IYmpsa2dVSkpaWUJWVmN1UmFZdHl5ejFjIn0..VYZcPeujLulFYl1ZOpdSXhafpRAM6qmWK48le-g30m9DGyfxsHqBSVERxBa1IqK5jTareyXvXWo2nVBCC67x_P6OfqtZRcMTUCaWQ3x_m6soScI6oCZBdEXjLPR8z_40ihhckO1aIdcqmI1ULlhrp_obPs00VdvKnXM8u9qZIhaOvb79-xYGx8CWd6LY2Foe_J_sW8YjRwEMeeVucqWBVdfJVpJ_gyuQgfigfV78ufjaKyIjy-FZ2t-ZGx5iT-ZsIaxK8eUqStljyQzDF5vZl3ZtJrdDgPZPDgU0ntbllSFSDMPt6i2BgvqQWh_ry4IV8mE2pPFYDicF3Rabzz982w'
        }
      };

      try {
        const response = await axios(config);
        const entities = response.data.model.entities;

        const yearsSet = new Set();
        const states = {};

        entities.forEach(item => {
          const { year, total_yield, state } = item.data;
          yearsSet.add(year);

          if (!states[state]) {
            states[state] = {};
          }
          states[state][year] = total_yield;
        });

        const years = Array.from(yearsSet).sort();
        const stateNames = Object.keys(states);

        setChartData({ years, states, stateNames });
      } catch (error) {
        console.error('Error fetching data:', error);
      }
    };

    fetchData();
  }, []);

  const getOption = () => {
    const colorList = chartData.stateNames.map((state, index) => {
      const hue = (index / chartData.stateNames.length) * 360;
      return `hsl(${hue}, 70%, 50%)`;
    });

    const series = chartData.stateNames.map((state, index) => ({
      name: state,
      type: 'line',
      stack: 'total',
      data: chartData.years.map(year => chartData.states[state][year] || 0),
      itemStyle: {
        color: colorList[index],
      }
    }));

    return {
      title: {
        text: '',
      },
      tooltip: {
        trigger: 'axis',
        axisPointer: {
      
        },
        formatter: (params) => {
          let tooltipText = params[0].axisValue;
          params.forEach(item => {
            tooltipText += `<br/>${item.marker} ${item.seriesName}: ${item.value}`;
          });
          return tooltipText;
        }
      },
      legend: {
        data: chartData.stateNames,
        textStyle: {
          color: (params) => {
            const stateIndex = chartData.stateNames.indexOf(params.name);
            return colorList[stateIndex];
          }
        },
      },
      xAxis: {
        type: 'category',
        data: chartData.years,
      },
      yAxis: {
        type: 'value',
      },
      series: series,
    };
  };

  return (
    <ReactECharts option={getOption()} style={{ height: 400, width: '100%' }} />
  );
};

export default YieldByYearAndStateChart;
