...
const YieldByCropSeasonChart = () => {
  const [chartData, setChartData] = useState({ years: [], dataBySeason: {} });

  useEffect(() => {
    const fetchData = async () => {
      const config = {
        method: 'get',
        url: 'https://ig.gov-cloud.ai/pi-get-data-quarkus/v1.0/bigQuery/66b209fcb7ea59463e796648/data?size=10000',
        headers: {
      ...
      try {
        ...
        entities.forEach(item => {
          const { year, total_yield, crop_season } = item.data;

          if (!years.includes(year)) {
            years.push(year);
          }

          if (!dataBySeason[crop_season]) {
            dataBySeason[crop_season] = [];
          }

          dataBySeason[crop_season].push({ year, total_yield });
        });

        setChartData({ years, dataBySeason });
      } catch (error) {
  ...
  const getOption = () => {
    const series = Object.keys(chartData.dataBySeason).map(season => ({
      name: season,
      type: 'line',
      data: chartData.dataBySeason[season].map(item => item.total_yield),
    }));
    ...
    return {
      ...
      legend: {
        data: Object.keys(chartData.dataBySeason),
      },
      xAxis: {
        type: 'category',
...
export default YieldByCropSeasonChart;
