...
const AverageMoistureContentChart = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const config = {
        method: 'get',
        url: 'https://ig.gov-cloud.ai/pi-get-data-quarkus/v1.0/bigQuery/66b2109cb7ea59463e79664c/data?size=10000',
        headers: {
      ...
      try {
        ...
        entities.forEach(item => {
          const { year, average_moisture, crop_types } = item.data;
          ...
          dataByCropType[crop_types].push({ year, average_moisture });
        });
  ...
  const generateInsights = (dataByCropType) => {
    let maxAverageMoisture = 0;
    let cropWithMaxMoisture = "";

    Object.keys(dataByCropType).forEach(cropType => {
      const totalMoisture = dataByCropType[cropType].reduce((sum, item) => sum + item.average_moisture, 0);
      const averageMoisture = totalMoisture / dataByCropType[cropType].length;

      if (averageMoisture > maxAverageMoisture) {
        maxAverageMoisture = averageMoisture;
        cropWithMaxMoisture = cropType;
      }
    });

    const insight = `${cropWithMaxMoisture} has the highest average moisture content over the years.`;
    const cta = `Consider optimizing the moisture levels for ${cropWithMaxMoisture} to improve yield.`;
  ...
  const getOption = () => {
    const series = Object.keys(chartData.dataByCropType).map(cropType => ({
      name: cropType,
      type: 'line',
      data: chartData.dataByCropType[cropType].map(item => item.average_moisture),
      itemStyle: {
  ...
  return (
    <div className="average-moisture-content-chart-container" style={{ width: "100%", height: "100%" }}>
                ...
                {chartData.years.map((year, yearIndex) => (
                  <tr key={yearIndex}>
                    ...
                    {Object.keys(chartData.dataByCropType).map((cropType, cropIndex) => (
                      <td key={cropIndex}>
                        {chartData.dataByCropType[cropType].find(item => item.year === year)?.average_moisture || '-'}
                      </td>
                    ))}
                  </tr>
                ))}
...
export default AverageMoistureContentChart;
